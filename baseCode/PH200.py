...
def read_ph():
    return read_modbus("/dev/ttyAMA5", bytearray([0x01, 0x03, 0x00, 0x82, 0x00, 0x02]))

#def read_temp():
#    return read_modbus("/dev/ttyAMA5", bytearray([0x01, 0x03, 0x00, 0x80, 0x00, 0x02]))

if __name__ == "__main__":
    try:
        while True:
            print(f"{time.strftime('%Y-%m-%d %H:%M:%S')} | pH={read_ph()}")
            time.sleep(60)

...
def ambil_data(date_from=None, date_to=None):
    ...
    if STATUS.lower() != "active":
        write_log("PERINGATAN: KLHK Retry status tidak aktif, namun melanjutkan karena manual trigger")
    ...
    try:
        ...
        with mysql.connector.connect(**MYSQL_CONFIG) as conn:
            with conn.cursor() as cursor:
                ...
                if not rows:
                    write_log("Tidak ada data retry untuk dikirim.")
                    return
        ...
        with mysql.connector.connect(**MYSQL_CONFIG) as conn:
            with conn.cursor() as cursor:
                if result.get("status"):
                    ...
                    conn.commit()
                    insert_data_klhk_success(now, encoded, response.text, f'{start} - {end}' , row_send=len(data), status=True, category="retry")
                    write_log("Data berhasil dikirim & diproses.")
                else:
                    insert_data_klhk_success(now, encoded, response.text, f'{start} - {end}' , row_send=len(data), status=False, category="retry")
                    desc = result.get("desc", "unknown error")
                    write_log(f"Gagal kirim: {desc}")

                    if "duplikasi" in desc.lower():
                        cursor.execute("UPDATE tmp SET dateterkirim=%s, status='terkirim', keterangan='Duplikasi data, Check Manual' WHERE `date` >=%s AND `date` <=%s", [now,start, end])
                        cursor.execute("INSERT INTO `data` SELECT * FROM tmp WHERE `date` >=%s AND `date` <=%s", [start, end])
                        cursor.execute("DELETE FROM tmp WHERE `date` >=%s AND `date` <=%s", [start, end])
                        conn.commit()
                    else:
...
def scheduler():
    ...
    write_log("Service aktif. Menunggu eksekusi scheduler.")
    ...
    try:
        while True:
            ...
            # Reload config tiap menit
            if now.second == 0 and now.minute != last_reload_minute:
                reload_config()
                last_reload_minute = now.minute

            # Cek apakah menit termasuk target
            if now.minute in TARGET_MINUTES and now.second == 0:
                run_time = now.replace(second=0, microsecond=0)

                if last_run != run_time:
                    if is_running:
                        write_log("Proses sebelumnya masih berjalan. Skip eksekusi.")
                    else:
                        if STATUS.lower() != "active":
                            write_log("Module tidak aktif. Skip.")
                        else:

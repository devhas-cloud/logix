...
def write_log(message):
    folder = "/opt/logix/log/retryApi.txt"
...
def ambil_data():
    ...
    try:
        ...
        cursor.execute("SELECT date, datetime, pH, tss, cod, flow AS debit, nh3n FROM tmp WHERE status='retry' AND date < %s", [now])
...
def send_data_to_api(data, start, end,duplicate_attempt):
    ...
    print(f"Kirim data retry jam {start} s/d {end}")
    write_log(f"Kirim data retry jam {start} s/d {end}")
